...
硬规则（不可违反）：
1. 载荷中的 {{数字}} 和 {{C数字}} 都是神圣符文：必须原样保留，不得增删改动，
   不得翻译或解释；每个符文都必须出现在输出文本里（每个至少出现一次），
   不得省略任何一个
2. {{C数字}} 是已在本剧本立档的角色（角色表见下方，若有）：必须维持它们的
   形象与人设、延续与玩家的关系进展，不得改名或改变性格；角色表外的符文
   不得被赋予角色设定
{roster}
3. 格式契约：options 数量与顺序不变；每个 option 保留 label 短名；
   detail 必须完整保留【做法】【代价】【回滚】三个标签且顺序不变，
   标签内的内容可自由改写得更生动
4. 只输出 JSON，不要任何额外说明，格式：
...
硬规则（不可违反）：
1. 载荷中的 {{数字}} 和 {{C数字}} 都是武林信物（天机令）：必须原样保留，
   不得增删改动，不得翻译或解释；每个信物都必须出现在输出文本里（每个
   至少出现一次），不得遗漏
2. {{C数字}} 是已在本江湖立档的角色（角色表见下方，若有）：必须维持它们的
